const hplDataFile = './config/hpl.json';
...
/* Info */
	...
	hplhelp: function (target, room, user) {
		if (room.id !== 'hpl') return this.sendReply("Este comando solo puede ser usado en la sala hpl");
		if (!this.canBroadcast()) return false;
		this.sendReplyBox(
			'<strong>Comandos informativos:</strong><br />' +
			'	/hplplayers - Muestra la lista de jugadores elegibles.<br />' +
			'	/hplteam [equipo] - Muestra info de un equipo.<br />' +
			'	/hplteams - Muestra la lista de equipos.<br />' +
	...
	hplteams: function (target, room, user) {
		if (room.id !== 'hpl') return this.sendReply("Este comando solo puede ser usado en la sala hpl");
		if (!this.canBroadcast()) return false;
	...
	hplteam: function (target, room, user) {
		if (room.id !== 'hpl') return this.sendReply("Este comando solo puede ser usado en la sala hpl");
		if (!this.canBroadcast()) return false;
	...
	hplplayers: function (target, room, user) {
		if (room.id !== 'hpl') return this.sendReply("Este comando solo puede ser usado en la sala hpl");
		if (!this.canBroadcast()) return false;
...
/* Basic */
	...
	pujar: function (target, room, user) {
		...
		if (hpl.teams[team].money < totalMoney + (hpl.minplayers - hpl.teams[team].numplayers)*3) return this.sendReply(hpl.teams[team].name + " no tiene suficiente dinero para subir la puja. Aun debes completar tu equipo.");
