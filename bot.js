    ...
    processBotCommands: function (user, room, connection, message) {
        ...
        if ((message.charAt(0) === '.' && Object.keys(Bot.commands).join(' ').toString().indexOf(cmd) >= 0 && message.substr(1) !== '') && !Bot.config.debug) {
            ...
            if ((now - user.lastBotCmd) * 0.001 < 5) {
                connection.sendTo(room, 'PxP Bot: Please wait ' + Math.floor((5 - (now - user.lastBotCmd) * 0.001)) + ' seconds until the next command.');
                return true;
            }
...
var commands = {
    ...
    about: function (target, room, user) {
		if (!this.can('joinbattle')) return this.sendPm('Permitame usted presentarme, yo soy el Bot del servidor Pokexperto y te ayudaré en todo lo que pueda, si tienes alguna duda sobre mis funciones, escribe: .guide ');
		this.sendReply('Permitame usted presentarme, yo soy el Bot del servidor Pokexperto y te ayudaré en todo lo que pueda, si tienes alguna duda sobre mis funciones, escribe: .guide');
	},

		foro: function (target, room, user) {
		if (!this.can('joinbattle')) return this.sendPm('Foros PxP: http://www.pokexperto.net/foros/index.php  , Twitter Foros PxP: https://twitter.com/ForosPxP    PxP Web: http://www.pokexperto.net');
		this.sendReply('Foros PxP: http://www.pokexperto.net/foros/index.php  , Twitter Foros PxP: https://twitter.com/ForosPxP    PxP Web: http://www.pokexperto.net');
	},
    ...
    guide: function (target, room, user) {
        ...
        this.sendReply('Lista de comandos del PxP Bot: ' + commands);
    },
    ...
    seen: function (target, room, user, connection) {
        ...
        if (!parse.chatData[toId(target)] || !parse.chatData[toId(target)].lastSeen) {
            return this.sendPm('El usuario ' + target.trim() + ' nunca fué visto en las salas.');
        }
        return this.sendPm(target.trim() + ' fué visto por ultima vez ' + parse.getTimeAgo(parse.chatData[toId(target)].seenAt) + ' , ' + parse.chatData[toId(target)].lastSeen);
    },

    salt: function (target, room, user) {
        if (!global.salt) global.salt = 0;
        salt++;
        this.sendReply(salt + '% PxP Bot.');
    },

    whois: (function () {
        var reply = [
            "Es solo otro usuario del servidor Pokexperto",
            "Un entrenador muy competitivo",
            "Un entrenador respetable",
            "Normalmente, un mal usuario",
            "Normalmente, un buen usuarip",
            "Alguien que es mucho mejor que tu, Y LO SABES",
            "Una persona maravillosa",
            "Una bellisima persona",
            "Una persona que no tiene vicios chungos",
            "Un lider al que todos adoran",
            "Un seguidor de Lord Helix",
            "Una persona encantadora",
            "Una persona con una personalidad personalmente personal",
            "Un adicto a oler calcetines sucios y olerse sus propios pedos",
            "Un jugador novato",
        ];
        ...
        return function (target, room, user) {
            ...
            if (target === config.userid()) message = '¡Ese soy yo! ¡El que mas mola!.';
			...
			if (target === 'maestroarena') message = 'Huele a pis porque a los gatos les encanta mearse en Arena﻿';
